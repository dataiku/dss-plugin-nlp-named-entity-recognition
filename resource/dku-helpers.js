    ...
    return fetch(getUrl(path) + "?" + $.param(args), {
      ...
      .then((response) => {
        ...
        if (response.status == 502) {
          ...
          response
            .text()
            .then((text) =>
              dataiku.webappMessages.displayFatalError(
      ...
      .catch(function (error) {
        if (
          ...
          // little hack, backend not started should be handled elsewhere
...
dataiku.webappMessages = (function () {
  function displayFatalError(err) {
    const errElt = $(
      '<div class="fatal-error" style="margin: 30px auto; text-align: center; color: var(--error-red)"></div>'
    );
    errElt.text(err);
